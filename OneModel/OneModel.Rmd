...
```{r}
psychometricM <- function(intensity,m,sd,guess,lapse){
  # handle arguments
  if(missing(guess)) {
    guess <- .5
  }
  ...
  z <- drop(outer(intensity, m, `-`) / rep(sd, each = nrow(outer(intensity,post$threshold[post$subject==i],`-`))))
...
```{r}
traceplot(oneMdl,pars=c('b_threshold[1]','b_spread[1]','b_lapse[1]','b_threshold[2]','b_spread[2]','b_lapse[2]','b_threshold[3]','b_spread[3]','b_lapse[3]','sigma_log_threshold','sigma_log_spread','sigma_logit_lapse','threshold[1]','spread[1]','lapse[1]'))
```
...
```{r}
pairs(oneMdl,pars=c('b_threshold[1]','b_spread[1]','b_lapse[1]','b_threshold[2]','b_spread[2]','b_lapse[2]','b_threshold[3]','b_spread[3]','b_lapse[3]','sigma_log_threshold','sigma_log_spread','sigma_logit_lapse','threshold[1]','spread[1]','lapse[1]'))
```
...
```{r}
plot(oneMdl,pars=c('b_threshold[1]','b_spread[1]','b_lapse[1]','b_threshold[2]','b_spread[2]','b_lapse[2]','b_threshold[3]','b_spread[3]','b_lapse[3]','sigma_log_threshold','sigma_log_spread','sigma_logit_lapse','threshold[1]','spread[1]','lapse[1]'))
```
...
Next, we'll want to pull our posterior samples into a tidy (i.e., long) data table.

```{r}
# assign posterior samples to tidy tibble
post <- mdl %>% 
  spread_draws(b_threshold[condition],b_spread[condition],b_lapse[condition],sigma_log_threshold,sigma_log_spread,sigma_logit_lapse,threshold_zoffset[subject],spread_zoffset[subject],lapse_zoffset[subject],threshold[subject],spread[subject],lapse[subject])
# print top 15 lines
post %>% head(15)
```

We can use this data format to show estimated psychometric functions for each participant, color-coded by visualization condition.

```{r}
# generate a range of intensity values to plot
n_intensity <- 101
intensity <- seq_range(useData$intensity, n = n_intensity)
# calculate possible PFs for every draw in post
pfs <- psychometricM(intensity,post$threshold,post$spread,0.5,post$lapse)
# add PFs and intensities to tibble of posterior samples
post <- add_column(post,pf = split(pfs, col(pfs)), intensity = list(intensity)) %>% unnest()


# calculate 100 possible PFs for each subject
# iterations <- 100
# n_subjects <- nlevels(useData$subject)
# intnsty <- array(0,dim=n_intensity*iterations*n_subjects) # preallocate
# pf <- array(0,dim=n_intensity*iterations*n_subjects)
# iter <- array(0,dim=n_intensity*iterations*n_subjects)
# subj <- array(0,dim=n_intensity*iterations*n_subjects)
# cond <- array(0,dim=n_intensity*iterations*n_subjects)
# for (i in 1:n_subjects) {
#   for (j in 1:iterations) {
#     # starting and ending indices
#     start <- (i - 1)*iterations*n_intensity + (j - 1)*n_intensity + 1
#     end <- start + n_intensity - 1
#     # fill in vectors that will become columns of data table
#     intnsty[start:end] <- intensity
#     pf[start:end] <- psychometric(intensity,post$threshold[post$subject==i & post$.draw==j],post$spread[post$subject==i & post$.draw==j],0.5,post$lapse[post$subject==i & post$.draw==j])
#     iter[start:end] <- rep(j,n_intensity)
#     subj[start:end] <- rep(i,n_intensity)
#     iter[start:end] <- rep(post$condition[post$subject==i && post$.iteration==j],n_intensity)
#   }
# }

# plot pfs based on posterior samples
# post %>%
#   mutate(pf = psychometric(intensity,threshold,spread,lapse))
# useData %>%    # get the values we want from the raw data for our prediction grid
#   data_grid(intensity = seq_range(intensity, n = 101), condition, subject) %>%
#   add_fitted_draws(mdl, n = 100) %>%    # sample 100 fits from the posterior
#   ggplot(aes(x = intensity, y = correct)) +
#   geom_line(aes(y = psychometric(intensity,threshold,spread,lapse), group = .draw), alpha = 0.2, color = condition) +
#   geom_point(data = useData) +
#   facet_wrap(~ subject)
```
