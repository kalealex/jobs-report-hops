...
e2m <- map2stan(
  alist(
    ...
    scaledProbit(p,threshold[subject],spread[subject],0.5,lapse[subject]) <- a + br*ratio + bh*hops + bf*fast + bl*lines, # likelikehood function w/ scaled probit link
    ...
    c(a,br,bh,bf,bl) ~ dnorm(0,10) # weak priors for model coefficients
  ),
  data = e2df,
  chains = 4,
  cores = 2,
