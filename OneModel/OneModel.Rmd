...
model {
  ...
  for (i in 1:N) {
    ...
    real lm;
    lm = a + bi*intensity[i] + bh*hops[i] + bf*fast[i] + bl*lines[i];
...
```{r}
# run model 
# (I used chains = 4, warmup = 500, & iter = 150 when creating the model in my environment.
# I cut this down to save time during Knit.)
e2m <- stan(
  model_code = stanCode,
  data = e2data,
  chains = 2,
...
```{r}
traceplot(e2m,pars=c('a','bi','bh','bf','bl'))
```

Here, we just want to see that the simulation is not getting stuck on particular parameter values. This plot looks fine.
...
```{r}
pairs(e2m,pars=c('a','bi','bh','bf','bl'))
```

This does not look good. We may need to rethink the parameterization of this model in order to tease apart the model coefficients for our three visualization conditions. Currently, our model fit is probably not properly estimating these parameters due to non-identifiability.
...
Note that the credibility intervals (CIs) on these estimates are very wide. We should expect that a Bayesian hierarchical model, designed to preserve uncertainty in PF fits at the level of estimating population parameters, will be more conservative and have wider CIs However, this might also reflect problems with identifiability in our model.
